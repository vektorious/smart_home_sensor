...
This is the standard build. You solder four wires to the BME680, connect it to the board, flash a prebuilt firmware image from your browser, and configure the device through its own setup page. No Arduino IDE, no libraries and no account needed.
...
Cut one end off each Dupont wire and strip about 3 mm of insulation. Twist the exposed copper strands together. Keep the female socket on the other end; it plugs onto the board's header pins.

![The four wires cut and stripped, ready to solder](img/wires_stripped_02.jpg)

Use the four pads labelled **VCC, GND, SDA, and SCL**. Push each stripped end through its hole **from the back**, the plain side without the components, and bend it flat against the labelled front so the wire runs off the side edge of the module, in the plane of the board. Soldering them this way keeps the wires out of the way of both the sensor and the enclosure.

<div align="center">

<img src="img/wires_through_pads_03.jpg" alt="Stripped wire ends pushed through the pad holes and bent flat, before soldering" width="700">
...
<img src="img/wires_soldered_04.jpg" alt="The four wires soldered to the BME680 pads, wires leaving flat to the side" width="700">

</div>

<div align="center">

<img src="img/wires_soldered_side_05.jpg" alt="Side view of the soldered joints, showing the wires staying in the plane of the module" width="700">
...
Before plugging in the USB cable, check that VCC connects to **3V3, not 5V**, and that SDA and SCL are not swapped. Swapping those two signal wires is a common reason the board cannot detect the sensor.

| BME680 pin | VCC | GND | SDA    | SCL    |
| ---------- | --- | --- | ------ | ------ |
| Board pin  | 3V3 | GND | GPIO 3 | GPIO 2 |
...
## 3. Flash and test the sensor

Before putting the board into the enclosure, flash it and verify that the wired BME680 is working. Wiring problems are much easier to fix while the board and sensor are still accessible.

1. Open the [web flasher](https://alexanderkutschera.com/smart_home_sensor/).
2. Pick an image. In a workshop, choose the one your instructor recommends: it usually carries the event's dashboard settings already, so there is less to fill in later. Otherwise pick **diy-sensor.org** to publish to the public dashboard, or **Home Assistant (MQTT)** if you already run Home Assistant with an MQTT broker.
...
If the port list is empty, start the board in flashing mode: unplug it, hold **BOOT**, plug the cable back in while holding the button, then release it after about 2 seconds. If the board still does not appear, try another USB data cable.
...
If you printed the enclosure, assemble it now. It keeps the display visible and holds the BME680 above the board, away from the heat the ESP32 and the backlight give off, which can otherwise distort the temperature and humidity readings (see [`background_information.md`](background_information.md)).
...
3. **Lower inlay**: separates the board from the sensor above it.
4. **Upper inlay**: holds the BME680 under the lid.
...
Lay the styrofoam over the back of the sensor. It fills the space between sensor and board and reduces heat transfer from the ESP32.
...
3. Tap **Configure WiFi**, pick the network, enter the password, and save. Every page here is served by the sensor itself, so give it a few seconds. Saving the Wi-Fi credentials takes longer because the board connects to the selected network. Tapping a second time only queues a second request. If your network is missing, check that it supports **2.4 GHz**. The ESP32-C6 cannot connect to a 5 GHz-only network.
...
On a diy-sensor.org image, a green **✓ 201** with **reading stored** means the server accepted your reading and the device is ready. Press the button again and you will see **200** instead: 201 means the device claimed its ID with that first reading, 200 that it added another. Both mean stored.

On a Home Assistant image, enter the broker host, port, and credentials under **Setup** and save them before sending a test reading. The test checks the connection to your broker. Further MQTT instructions are in [`build_instructions_extended.md`](build_instructions_extended.md).

Press **Finish setup** when the test passes. The portal also closes on its own after 10 minutes.

> Your **device ID** is printed on the display and on the setup page. The default ID is derived from the chip and stays the same across resets and reflashes. You can override it under **Setup**; a factory reset restores the default ID.
...
> ⚠️ **The current enclosure covers the RESET button.** Until a revised case opens it up, reaching it means sliding the board out of the housing. You can also enter setup without using the button: at power-up, the device opens setup mode if it cannot connect to a saved Wi-Fi network. Power it up out of range of the saved network, or with that network switched off. Then join the setup Wi-Fi network shown on the display. (A network that drops *while* the device is running does not reopen setup; it reconnects in the background instead.)

---

## 6. Find your readings
...
- **Compare temperature** against a separate thermometer after the device has warmed up for 20–30 minutes. Yours will read high: the ESP32 and the backlight warm the sensor board. The setup page has a **Temperature offset** field to correct it. It already defaults to 5 °C, so add whatever error is left to that number rather than replacing it. If the display still reads 2.5 °C high, the offset becomes 7.5, not 2.5.
- **Watch IAQ accuracy.** It starts at 0 and works up to 3. Until it reaches 3, treat the IAQ value as provisional while BSEC learns the air-quality baseline in your room. Reaching 3 takes hours, and full convergence takes up to four days of running.
...
| **I need to change a setting** | Press **RESET twice quickly**. The display tells you when the window is open, then setup mode returns. In the current enclosure the button is covered. Either take the board out or power it up where its saved network is unreachable to open setup mode. |

The setup portal also offers three separate resets: *Forget Wi-Fi network*, *Clear IAQ calibration*, and *Factory reset* (settings + Wi-Fi). Only **Clear IAQ calibration** discards the air-quality calibration, which takes hours to rebuild. **Factory reset** also removes any custom device ID and restores the ID derived from the board.

---

## Taking it further

The device keeps publishing wherever it finds a Wi-Fi network it knows. Press RESET twice to enter your home Wi-Fi details in setup.

If a workshop image was used, note that it is tied to that event and comes off the flasher page afterwards. Removing the image from the flasher does not change the firmware on your board. It can keep publishing while the workshop API key remains active. You can read its write key from the setup page by pressing RESET twice.
